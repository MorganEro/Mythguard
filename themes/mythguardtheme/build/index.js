...
class Contract {
  ...
  updateProgramOptions(programs) {
    ...
    const sortedPrograms = [...programs].sort((a, b) => a.title.rendered.localeCompare(b.title.rendered));
    sortedPrograms.forEach(program => {
      this.programSelect.innerHTML += `<option value="${program.id}" ${currentValue === program.id.toString() ? 'selected' : ''}>${program.title.rendered}</option>`;
    });
  }
  updateGuardianOptions(guardians) {
    ...
    const sortedGuardians = [...guardians].sort((a, b) => a.title.rendered.localeCompare(b.title.rendered));
    sortedGuardians.forEach(guardian => {
      this.guardianSelect.innerHTML += `<option value="${guardian.id}" ${currentValue === guardian.id.toString() ? 'selected' : ''}>${guardian.title.rendered}</option>`;
    });
  }
  bindEvents() {
    document.addEventListener('click', e => {
...
class Search {
  ...
  renderSearchResults(data) {
    ...
    if (!generalInfo.length && !guardians.length && !programs.length && !locations.length && !events.length) {
