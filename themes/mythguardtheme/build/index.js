...
class Calendar {
  ...
  handleDateSelect(dateStr) {
    ...
    const contractEvents = this.dates.contracts.filter(contract => contract.date === dateStr).map(contract => ({
      ...contract,
      type: 'contract'
    }));

    // Get all gatherings on this date
    const gatheringEvents = this.dates.gatherings.filter(item => item.date === dateStr).map(gathering => ({
      ...gathering,
      type: 'gathering'
    }));
    ...
    if (events.length > 0) {
      ...
      if (legendElement) {
        legendElement.insertAdjacentHTML('afterend', `<div class="calendar-events">${eventList}</div>`);
      } else {
...
class Contract {
  ...
  setupDatePickers() {
    ...
    if (startDate) {
      (0,flatpickr__WEBPACK_IMPORTED_MODULE_1__["default"])(startDate, {
        ...commonConfig,
        enableTime: true,
        allowInput: true,
        altInput: true,
    ...
    if (endDate) {
      (0,flatpickr__WEBPACK_IMPORTED_MODULE_1__["default"])(endDate, {
        ...commonConfig,
        defaultHour: 17,
        // Default to 5 PM for end time
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = globalThis["webpackChunkmythguard_theme"] = globalThis["webpackChunkmythguard_theme"] || [];
